...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        if (isset($_POST['post-title']) && isset($_POST['post-content']) && isset($_POST['post-photo']) && isset($_POST['post-link'])) {
            ...
            $postStmt = $conn->prepare("INSERT INTO post (title,username, content, PHOTO_URL, LINK_URL) VALUES (?, ?, ?, ?, ?)");
            $postStmt->execute([$postTitle,$usernames, $postContent, $postPhoto, $postLink]);
        }
        ...
        if (isset($_POST['event-title']) && isset($_POST['event-description']) && isset($_POST['event-location']) && isset($_POST['event-datetime'])) {
            ...
            echo $eventTitle;
            $eventStmt = $conn->prepare("INSERT INTO events (title,username, description, location, datetime) VALUES (?, ?, ?, ?, ?)");
            $eventStmt->execute([$eventTitle,$usernames, $eventDescription, $eventLocation, $eventDatetime]);
        }
